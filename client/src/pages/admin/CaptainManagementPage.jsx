                ...
                {loading ? <tr><td colSpan="8" className="py-12 text-center text-slate-500">Loading captain tasks...</td></tr> : tasks.length === 0 ? <tr><td colSpan="8" className="py-12 text-center text-slate-500">No captain tasks found.</td></tr> : tasks.map((task) => {
                  ...
                  return (
                    <tr key={task._id} className="hover:bg-slate-50">
                      ...
                      <td className="py-3.5 px-4"><strong className="text-slate-900">₹{amount.toLocaleString('en-IN')}</strong><span className={`block mt-1 text-[10px] font-bold ${isPaid ? 'text-emerald-700' : 'text-rose-600'}`}>{isPaid ? 'Paid' : `Due ₹${amount.toLocaleString('en-IN')}`}</span></td>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-2 sm:p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl sm:rounded-3xl w-full max-w-5xl max-h-[96vh] overflow-hidden shadow-2xl border-t-4 border-amber-400 flex flex-col">
            ...
            <form id="captain-task-form" onSubmit={handleSave} className="overflow-y-auto p-4 sm:p-6 space-y-5">
              ...
              <section className="rounded-2xl border border-amber-200 bg-white p-4 sm:p-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  <div><label className="block text-[11px] font-bold text-slate-700 mb-1">Payment Status *</label><select value={formData.paymentStatus} onChange={(event) => setFormData({ ...formData, paymentStatus: event.target.value })} className="w-full px-3.5 py-3 rounded-xl border border-slate-300 text-sm font-bold"><option value="Pending">Pending Due</option><option value="Paid">Paid</option></select></div>
            ...
            <div className="px-4 sm:px-6 py-3 border-t border-slate-200 bg-white flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 shrink-0">
              <div className="text-[11px] font-black text-slate-500 uppercase">Total: <span className="text-slate-900">₹{Number(formData.salaryAmount || 0).toLocaleString('en-IN')}</span><span className="mx-2 text-slate-300">|</span><span className={formData.paymentStatus === 'Paid' ? 'text-emerald-700' : 'text-rose-600'}>{formData.paymentStatus === 'Paid' ? 'Paid' : 'Due'}</span></div>
